...
#   cd $SHARED_HOST_FOLDER && git clone https://github.com/marcosfermin/tuliocp.git && cd tuliocp && git checkout ..branch..
#
...
array_walk($containers, function (&$element) {
	...
	$element["hst_installer"] = "tulio-install-ubuntu.sh";
	...
	if ($element["os"] == "ubuntu16.04") {
		...
		$element["hst_installer"] = "tulio-install-debian.sh";
		$lxc_name .= "d8";
...
function getTulioVersion($branch) {
	...
	if ($branch === "~localsrc") {
		...
		$control_file = file_get_contents(
			"https://raw.githubusercontent.com/marcosfermin/tuliocp/${branch}/src/deb/tulio/control",
		);
	}

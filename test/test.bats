...
@test "DNS: Add domain record SRV" {
    run v-delete-dns-record $user $domain 50
    run v-add-dns-record $user $domain '_test_domain' SRV '10 5 443 mx.tuliocp.com'  '' 50
    assert_success
    refute_output

    assert_file_contains "$HOMEDIR/$user/conf/dns/${domain}.db" "mx.tuliocp.com."

    run v-change-dns-record $user $domain 50 '_test.domain' SRV '10 5 443 mx.tulio.com'
    assert_success
    refute_output
...
@test "System: Add Mail DNSBL (Simple Syntax)" {
  ...
  assert_success
  refute_output
  assert_file_exist $TULIO/conf/dnsbl.conf
  assert_file_exist /etc/exim4/dnsbl.conf
  assert_file_contains $TULIO/conf/dnsbl.conf "sbl.spamhaus.org"
...
@test "System: Add Mail DNSBL (Complex Syntax)" {
  ...
  assert_success
  refute_output
  assert_file_contains $TULIO/conf/dnsbl.conf "zen.spamhaus.org!=127.255.255.252,127.255.255.254,127.255.255.255"

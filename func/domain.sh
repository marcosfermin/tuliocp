...
is_valid_extension() {
	...
	if [ ! -e "$TULIO/data/extensions/public_suffix_list.dat" ]; then
		mkdir -p "$TULIO/data/extensions/"
		chmod 750 "$TULIO/data/extensions/"
		if /usr/bin/wget --tries=3 --timeout=15 --read-timeout=15 --waitretry=3 --no-dns-cache --quiet -O "$TULIO/data/extensions/public_suffix_list.dat.tmp" "$psl"; then
			mv "$TULIO/data/extensions/public_suffix_list.dat.tmp" "$TULIO/data/extensions/public_suffix_list.dat"
		else
			rm -f "$TULIO/data/extensions/public_suffix_list.dat.tmp"
		fi
	elif find "$TULIO/data/extensions/public_suffix_list.dat" -mtime +7 2> /dev/null | grep -q .; then
		mv "$TULIO/data/extensions/public_suffix_list.dat" "$TULIO/data/extensions/public_suffix_list.dat.save"
		if /usr/bin/wget --tries=3 --timeout=15 --read-timeout=15 --waitretry=3 --no-dns-cache --quiet -O "$TULIO/data/extensions/public_suffix_list.dat.tmp" "$psl"; then
			mv "$TULIO/data/extensions/public_suffix_list.dat.tmp" "$TULIO/data/extensions/public_suffix_list.dat"
			rm -f "$TULIO/data/extensions/public_suffix_list.dat.save"
		else
			rm -f "$TULIO/data/extensions/public_suffix_list.dat.tmp"
			mv "$TULIO/data/extensions/public_suffix_list.dat.save" "$TULIO/data/extensions/public_suffix_list.dat"
		fi
	fi
	if [ ! -e "$TULIO/data/extensions/public_suffix_list.dat" ]; then
	...
	exten=$(grep -Fx "$extension" "$TULIO/data/extensions/public_suffix_list.dat")
}

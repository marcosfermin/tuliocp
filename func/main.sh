...
#     - Tulio's own path/binary-location globals (BIN, HOMEDIR,
...
#    ROOT_USER is deliberately not in this list: tulio.conf legitimately
...
# tulio.conf during main.sh's own bootstrap), so it must fail closed on
...
source_conf() {
	...
	reserved+=' TULIO BIN HOMEDIR BACKUP USER_DATA WEBTPL MAILTPL DNSTPL RRD SENDMAIL'
	reserved+=' TULIO_INSTALL_DIR TULIO_COMMON_DIR TULIO_BACKUP TULIO_PHP TULIO_GIT_REPO'
	reserved+=' TULIO_THEMES TULIO_THEMES_CUSTOM SCRIPT CHECK_RESULT_CALLBACK user'
...
TULIO_BACKUP="/root/hst_backups/$(date +%d%m%Y%H%M)"
...
update_user_value() {
	key="${2//$/}"
	lnr=$(grep -m 1 -n "^$key='" $TULIO/data/users/$1/user.conf | cut -f 1 -d ':')
	if [ -n "$lnr" ]; then
...
is_dns_record_format_valid() {
	...
	json_from_php=$(
		$TULIO_PHP "$TULIO/func/internal/dns_record_validator.php" "$1" "$rtype" "$priority"
	)

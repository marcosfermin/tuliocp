						...
						<?php if ($v_mysql == "yes") { ?>
							...
							<div class="u-mb10">
								<label for="v_pma_restrict" class="form-label">
									<?= tohtml( _("Restrict phpMyAdmin to Tulio login")) ?>
									<span class="hint">
										<?= tohtml( _("Blocks phpMyAdmin's own login form; only reachable via the phpMyAdmin link inside Tulio (requires SSO above).")) ?>
									</span>
								</label>
								<select
									class="form-select"

	...
	<div class="toolbar-inner">
		<div class="toolbar-buttons">
			<?php if ($read_only !== "true") { ?>
				...
				<?php } ?>
				<?php if (ipUsed()) { ?>
					<a target="_blank" href="https://tuliocp.com/docs/panel/server-administration/databases.html#why-i-can-t-use-http-ip-phpmyadmin">
						...
						<ul class="units-table-row-actions">
							...
							<?php } ?>
								<?php if ($data[$key]['TYPE'] == 'mysql' && isset($_SESSION['PHPMYADMIN_KEY']) && $_SESSION['PHPMYADMIN_KEY'] != '' && !ipUsed()) { $time = time(); ?>
									<?php
										$tulio_sso_token = password_hash(
											$key . $user_plain . $_SESSION['user_combined_ip'] . $time . $_SESSION['PHPMYADMIN_KEY'],
											PASSWORD_DEFAULT,
										);
										$tulio_sso_url = $db_myadmin_link . "tulio-sso.php?" . http_build_query([
											"database" => $key,
											"user" => $user_plain,
											"exp" => $time,
											"tulio_token" => $tulio_sso_token,
										]);
									?>
									<li class="units-table-row-action shortcut-enter" data-key-action="href">
										<a
											class="units-table-row-action-link"
											href="<?= tohtml($tulio_sso_url) ?>"

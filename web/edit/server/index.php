...
if (!empty($_POST["save"])) {
	...
	// Set phpMyAdmin access restriction (Tulio SSO only, no direct login)
	if (empty($_SESSION["error_msg"])) {
		if (!empty($_POST["v_pma_restrict"])) {
			if ($_POST["v_pma_restrict"] == "yes" && $_SESSION["PMA_RESTRICT_ACCESS"] != "yes") {
				exec(TULIO_CMD . "v-add-sys-pma-restrict quiet", $output, $return_var);
				check_return_code($return_var, $output);
				unset($output);
				if (empty($_SESSION["error_msg"])) {
				...
				exec(TULIO_CMD . "v-delete-sys-pma-restrict quiet", $output, $return_var);
				check_return_code($return_var, $output);
				unset($output);
				if (empty($_SESSION["error_msg"])) {

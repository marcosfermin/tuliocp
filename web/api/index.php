...
function api_legacy(array $request_data) {
	...
	if (empty($request_data["hash"])) {
		...
		unset($output);
		exec(TULIO_CMD . "v-get-user-salt " . $user . " " . $v_ip . " json", $output, $return_var);
		...
		// Check user hash
		exec(
			TULIO_CMD . "v-check-user-hash " . $user . " " . $v_hash . " " . $v_ip,
			$output,
			$return_var,
		);
		unset($output);
		...
		$v_ip = quoteshellarg(get_real_user_ip());
		exec(
			TULIO_CMD . "v-check-api-key " . quoteshellarg($key) . " " . $v_ip,
			$output,
			$return_var,
		);
		unset($output);
	...
	if ($hst_cmd == "v-make-tmp-file") {
		...
		// Prepare command
		$cmdquery = TULIO_CMD . escapeshellcmd($hst_cmd);
...
if (isset($_POST["access_key"]) || isset($_POST["user"]) || isset($_POST["hash"])) {
	...
	api_error(
		405,
		"Error: data received is null or invalid, check https://tuliocp.com/docs/panel/server-administration/rest-api.html",
		"",
	);
}

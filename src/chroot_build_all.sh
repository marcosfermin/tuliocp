...
# Tulio packages on a single machine. hst_autocompile.sh only ever builds for
...
#
# Usage:
#   ./chroot_build_all.sh (--all|--tulio|--nginx|--php|--web-terminal) \
#   [--debug] [branch]
#
# Examples:
...
CHROOT_BASE_DIR='/var/lib/tuliocp-build-chroot'
ACTIVE_CHROOTS=()
...
usage() {
	echo "Usage:"
	echo "    $0 (--all|--tulio|--nginx|--php|--web-terminal) [--debug] [branch]"
	echo ""
...
prepare_chroot() {
	...
	if [ ! -f "$chroot_dir/.tuliocp-chroot-ready" ]; then
		...
		touch "$chroot_dir/.tuliocp-chroot-ready"
	fi
	...
	# same in-chroot path ($BUILD_DIR, e.g. /tmp/tuliocp-src). They must NOT
...
run_cross_build() {
	...
	# rootfs itself is left on disk (see .tuliocp-chroot-ready in
...
[ "$TULIO_DEBUG" = "true" ] && PKG_FLAGS="$PKG_FLAGS --debug"
...
# When done move all build packages located in /tmp/tuliocp-src-{os}-{release}-{arch}/deb to /tmp/tuliocp-src/deb/ to have all packages in one place

...
```bash
/usr/local/tulio/install/upgrade/manual/migrate_conf_to_debian_13.sh
```
...
You may see a few files listed, but you should ignore most of them. Configuration files managed or modified by Tulio (such as `sshd`, `dovecot`, `bind`, `exim4`, etc.) should always be ignored. If you are unsure whether a file needs to be reviewed or merged, it is safest to leave it unchanged.
...
Comment out [line 29](https://github.com/marcosfermin/tuliocp/blob/1ff8a4e5207aae1e241954a83b7e8070bcdca788/install/deb/proftpd/proftpd.conf#L29) in `/etc/profpd/prodtpd.conf`.
...
```bash
/usr/local/tulio/install/upgrade/manual/migrate_conf_to_ubuntu_26.04.sh
```

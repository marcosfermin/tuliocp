...
A Master server is where DNS zones are created, and a Slave server receives the zone via the API. Tulio can be configured as Master <-> Master or Master -> Slave. With a Master <-> Master configuration, each Master is also a Slave, so it could be considered as Master/Slave <-> Master/Slave.
...
::: info
It doesn't work if you try to sync via local network! See [Issue](https://github.com/marcosfermin/tuliocp/issues/4295) Make sure to use the public ip addresses
:::
...
::: danger
Removing or disabling the private key in Tulio will make the domain inaccessible.
:::

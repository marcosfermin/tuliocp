...
If you prefer to run cleanup from cron, create a script such as `/usr/local/sbin/tulio-purge-mail-folders`:
...
"$TULIO/bin/v-list-users" list | while read -r user; do
	while read -r domain; do
		...
		done < <("$TULIO/bin/v-list-mail-accounts" "$user" "$domain" plain 2> /dev/null | cut -f1 || true)
	done < <("$TULIO/bin/v-list-mail-domains" "$user" plain 2> /dev/null | cut -f1 || true)
done
```
...
```bash
chmod 750 /usr/local/sbin/tulio-purge-mail-folders
```

Run it manually first. By default, the script only prints what it would purge:

```bash
/usr/local/sbin/tulio-purge-mail-folders
```

Run the cleanup by setting `DRY_RUN=no`:

```bash
DRY_RUN=no /usr/local/sbin/tulio-purge-mail-folders
```

Then add a root cron job, for example to run daily at 03:30:

```text
30 3 * * * DRY_RUN=no /usr/local/sbin/tulio-purge-mail-folders >/var/log/tulio-purge-mail-folders.log 2>&1
```
...
1. Run `v-add-sys-mail-dnsbl HereYourQueryKey.zen.dq.spamhaus.net` to add your custom DQS key. (Note: TulioCP automatically prevents your Spamhaus Query key from leaking in bounce messages).
...
See: [Github](https://github.com/marcosfermin/tuliocp/pull/4365)

 ...
 * @codeCoverageIgnore
 */
class TulioAuth implements Service, AuthInterface {
	protected $permissions = [];

	protected $private_repos = false;

	protected $tulio_user = "";
	...
	public function user(): ?User {
		...
		exec($cmd . " " . quoteshellarg($this->tulio_user) . " json", $output, $return_var);

		if ($return_var == 0) {
			$data = json_decode(implode("", $output), true);
			$tulio_user_info = $data[$this->tulio_user];
			return $this->transformUser($tulio_user_info);
		}
	...
	public function transformUser($hstuser): User {
		$user = new User();
		$user->setUsername($this->tulio_user);
		$user->setName($this->tulio_user . " (" . $hstuser["NAME"] . ")");
		$user->setRole("user");
	...
	public function authenticate($username, $password): bool {
		# Auth is handled by Tulio
		return false;
	}

	public function forget() {
		// Logout return to Tulio
		return $this->getGuest();
	}
	...
	public function update($username, User $user, $password = ""): User {
		// Password change is handled by Tulio
		return $this->user();
	}

...
# Tulio Control Panel upgrade script for target version 1.9.5
...
# Ensure netplan configs with tulio in the name have restrictive permissions
for netplan_file in /etc/netplan/*tulio*; do
	[ -e "$netplan_file" ] || break
	echo "[ * ] Setting permissions on '$netplan_file' to 600"
	chmod 600 "$netplan_file"
done

# Fix: Tulio can't restart SpamAssassin from the Web UI because it tries to restart
...
for user in $("$TULIO"/bin/v-list-users list); do
	if [[ "$RESOURCES_LIMIT" == "yes" ]]; then
		"$TULIO"/bin/v-update-user-cgroup "$user"
	fi

	if [[ "$DISK_QUOTA" == "yes" ]]; then
		"$TULIO"/bin/v-update-user-quota "$user"
	fi
done

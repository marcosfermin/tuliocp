...
# Tulio Control Panel upgrade script for target version 1.10.3
...
if [[ "${FILE_MANAGER:-false}" == "true" ]] \
	&& [[ -f "$TULIO/web/fm/version" ]]; then

	fm_v="$(grep '^fm_v=' "$TULIO/install/upgrade/upgrade.conf" | cut -d"'" -f2)"
	current_fm_v="$(< "$TULIO/web/fm/version")"

...
# Tulio Control Panel upgrade script for target version 1.6.0
...
if [ -f "/etc/cron.d/tulio-sftp" ]; then
	rm /etc/cron.d/tulio-sftp
	echo "@reboot root sleep 60 && /usr/local/tulio/bin/v-add-sys-sftp-jail > /dev/null" > /etc/cron.d/tulio-sftp
fi
...
if [ $release = 'Ubuntu' ]; then
	if [ $ips -gt 1 ]; then
		add_upgrade_message "Warning: Please check your network configuration!\n\n A bug has been discovered that might affect your setup and can lead to issues after a system reboot. Please review your network configuration. See https://github.com/marcosfermin/tuliocp/pull/2612#issuecomment-1135571835 for more info regarding this issue!"
		$TULIO/bin/v-add-user-notification admin "Warning: Please check your network configuration!\n\n A bug has been discovered that might affect your setup and can lead to issues after a system reboot. Please review your network configuration. <a href='https://github.com/marcosfermin/tuliocp/pull/2612#issuecomment-1135571835'>More info</a>"
	fi
fi

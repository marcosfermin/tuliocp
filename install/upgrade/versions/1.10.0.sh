...
# Tulio Control Panel upgrade script for target version 1.10.0
...
cp -f "$TULIO"/install/deb/filemanager/filegator/configuration.php "$TULIO"/web/fm/configuration.php

if [ -f /etc/os-release ]; then
	# /etc/os-release defines its own $VERSION, which would otherwise
	# clobber Tulio's $VERSION since this script is sourced by the caller
	_TULIO_VERSION="$VERSION"
	source /etc/os-release
	VERSION="$_TULIO_VERSION"
	unset _TULIO_VERSION
fi
...
if $IS_DEBIAN13_OR_UBUNTU2604 && [[ "$dovecot_version" = "2.4" ]]; then
	if ! grep -q 'modified by Tulio' /etc/dovecot/dovecot.conf \
		|| ! grep -q 'ssl_server_cert_file = /usr/local/tulio' /etc/dovecot/conf.d/10-ssl.conf; then
...
if [[ -f "$phpmyadmin_conf" ]]; then
	...
	chown -R tuliomail:www-data /var/lib/phpmyadmin/tmp/
...
if $IS_UBUNTU2604; then
	if [[ -f /etc/sudoers.d/tulioweb ]] && grep -q '^Defaults:root !requiretty$' /etc/sudoers.d/tulioweb &> /dev/null; then
		echo "[ + ] Configuring sudoers to remove unsupported requiretty option"
		chmod 640 /etc/sudoers.d/tulioweb
		sed -i '/^Defaults:root !requiretty$/d' /etc/sudoers.d/tulioweb
		chmod 440 /etc/sudoers.d/tulioweb
	fi
fi

# Updating logrotate conf for Tulio
echo "[ * ] Updating logrotate conf for Tulio"
cp -f "$TULIO"/install/deb/logrotate/tulio /etc/logrotate.d/tulio

# Enhance - Update current composer installations
echo "[ * ] Updating composer for users:"
for huser in $("$TULIO/bin/v-list-users" list); do
	if [[ -f "/home/$huser/.composer/composer" ]]; then
		echo "      - $huser..."
		"$TULIO/bin/v-add-user-composer" "$huser" 2 yes &> /dev/null
	fi
done

...
# Tulio Control Panel upgrade script for target version 1.1.0
...
if [ -e "/etc/cron.d/tulio-sftp" ]; then
	if ! cat /etc/cron.d/tulio-sftp | grep -q 'root'; then
		echo "@reboot root /usr/local/tulio/bin/v-add-sys-sftp-jail" > /etc/cron.d/tulio-sftp
	fi
fi

...
# Tulio Control Panel upgrade script for target version 1.0.2
...
if [ -f "/etc/apache2/conf.d/tulio.conf" ]; then
	echo "[ * ] Removing old Apache configuration file from previous version of Tulio Control Panel..."
	rm -f /etc/apache2/conf.d/tulio.conf
fi
if [ -f "/etc/nginx/conf.d/tulio.conf" ]; then
	echo "[ * ] Removing old NGINX configuration file from previous version of Tulio Control Panel..."

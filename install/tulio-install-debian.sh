...
TULIO_BASE_VER="${TULIO_INSTALL_VER%%~*}"
if [[ "$TULIO_INSTALL_VER" == *"~"* ]]; then
	TULIO_CHANNEL="~${TULIO_INSTALL_VER#*~}"
else
	TULIO_CHANNEL=""
fi
...
case "$os" in
	...
	debian)
		...
		;;
	*)
		echo "Error: unsupported distribution for determining Tulio version ($os)"
		exit 1
		;;
esac
...
software="acl apache2 apache2-suexec-custom apache2-utils at awstats bc bind9 bsdmainutils bsdutils
  ...
  exim4 exim4-daemon-heavy expect fail2ban flex ftp git tulio=${TULIO_INSTALL_BUILD} tulio-nginx tulio-php tulio-web-terminal
...
if [ -d "/usr/local/tulio" ]; then
	check_result 1 "Tulio install detected. Unable to continue"
fi

type=$(grep "^ID=" /etc/os-release | cut -f 2 -d '=')
if [ "$type" = "ubuntu" ]; then
	check_result 1 "You are running the wrong installer for Ubuntu. Please run tulio-install.sh or tulio-install-ubuntu.sh instead."
...
# Verify that the TulioCP apt repository actually carries packages for this
# codename AND this architecture. Probing the repository root only proves that a
# web server answers, and probing the suite only proves the suite exists; neither
# says anything about whether a binary-$ARCH index is published. Check the
# signed suite index and the architecture's own Release/Packages files, all of
# which apt itself needs. This check is skipped when installing from local .deb
# files, which need no remote repo.
if [ -z "$withdebs" ] || [ ! -d "$withdebs" ]; then
	repo_probe() {
		curl -fsS --proto '=https' --tlsv1.2 --connect-timeout 10 --max-time 30 \
			--retry 2 --retry-delay 2 -o /dev/null "$1"
	}
	...
	repo_abort() {
		...
		echo -e "\e[91mInstallation aborted\e[0m"
		echo "===================================================================="
		echo -e "\e[33mERROR: the TulioCP apt repository does not publish $ARCH packages\e[0m"
		echo -e "\e[33mfor '$codename', or it could not be reached from this host.\e[0m"
		echo ""
		echo -e "\e[33mChecked: $1\e[0m"
		echo ""
		...
		echo ""
		check_result 1 "TulioCP apt repository unavailable for $codename/$ARCH"
	}

	repo_base="https://$RHOST/dists/$codename"
	for repo_url in \
		"$repo_base/InRelease" \
		"$repo_base/main/binary-$ARCH/Release" \
		"$repo_base/main/binary-$ARCH/Packages.gz"; do
		if ! repo_probe "$repo_url"; then
			repo_abort "$repo_url"
		fi
	done

	# The signed suite index must also advertise this architecture; a stale
	# binary-$ARCH directory left behind by an old publish would otherwise pass
	# the reachability checks above while apt still refuses to use it.
	if ! curl -fsS --proto '=https' --tlsv1.2 --connect-timeout 10 --max-time 30 \
		--retry 2 --retry-delay 2 "$repo_base/Release" \
		| grep -qE "^Architectures:.*\b$ARCH\b"; then
		repo_abort "$repo_base/Release (Architectures: must list $ARCH)"
	fi
fi
...
# Welcome message
echo "Welcome to the Tulio Control Panel installer!"
echo
...
if [ -n "$conflicts" ] && [ -z "$force" ]; then
	...
	if [ "$answer" = 'y' ] || [ "$answer" = 'Y' ]; then
		...
		check_result 1 "Tulio Control Panel should be installed on a clean server."
	fi
fi
...
install_welcome_message() {
	DISPLAY_VER=$(echo $TULIO_INSTALL_VER | sed "s|~alpha||g" | sed "s|~beta||g" | sed "s|~rc[0-9]*||g")
	echo
	...
	echo "========================================================================"
	echo
	echo "Thank you for downloading Tulio Control Panel! In a few moments,"
...
if [ "$apache" = 'yes' ]; then
	...
	import_apt_key https://packages.sury.org/apache2/apt.gpg /usr/share/keyrings/apache2-keyring.gpg
fi
...
apt-get -y purge tulio tulio-nginx tulio-php > /dev/null 2>&1
...
if [ -d "$withdebs" ]; then
	...
	software=$(echo "$software" | sed -e "s/tulio-web-terminal//")
	software=$(echo "$software" | sed -e "s/tulio=${TULIO_INSTALL_BUILD}//")
fi
...
if [ "$dovecot" = 'yes' ]; then
	...
	if [[ "$dovecot_version" = "2.4" ]]; then
		cp -f $TULIO_COMMON_DIR/dovecot/2.4/dovecot.conf /etc/dovecot/
		cp -f $TULIO_COMMON_DIR/dovecot/2.4/conf.d/* /etc/dovecot/conf.d/
	else
		cp -f $TULIO_COMMON_DIR/dovecot/2.3/dovecot.conf /etc/dovecot/
		cp -f $TULIO_COMMON_DIR/dovecot/2.3/conf.d/* /etc/dovecot/conf.d/
...
if [ "$sieve" = 'yes' ]; then
	...
	if [[ "$dovecot_version" = "2.4" ]]; then
		...
		cp -f "$TULIO_COMMON_DIR"/dovecot/2.4/sieve/* /etc/dovecot/conf.d
		...
		# Replace dovecot-sieve config files
		cp -f $TULIO_COMMON_DIR/dovecot/2.3/sieve/* /etc/dovecot/conf.d
	fi
...
if [[ $release -ge 13 ]]; then
	echo 'KexAlgorithms +diffie-hellman-group-exchange-sha256' > /etc/ssh/sshd_config.d/tulio-kex.conf
fi
...
# Add /usr/local/tulio/bin/ to PATH variable in .bashrc if it exists
[[ -f /root/.bashrc ]] && echo 'if [ "${PATH#*/usr/local/tulio/bin*}" = "$PATH" ]; then
    . /etc/profile.d/tulio.sh
fi' >> /root/.bashrc

# Add /usr/local/tulio/bin/ to PATH variable in .zshrc if it exists
[[ -f /root/.zshrc ]] && echo 'if [ "${PATH#*/usr/local/tulio/bin*}" = "$PATH" ]; then
    . /etc/profile.d/tulio.sh
fi' >> /root/.zshrc
...
You have successfully installed Tulio Control Panel on your server.
...
cat $tmpfile | $send_mail -s "Tulio Control Panel" $email
